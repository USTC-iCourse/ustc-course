...
(function() {
    ...
    function CheckForAutoSavedContent(editorInstance, autoSaveKey, notOlderThen) {
        ...
        if (localStorage.getItem(autoSaveKey)) {
            ...
            var current = editorInstance.getSnapshot();
            var loadedMessage = editorInstance.lang.autosave.savedContentLoaded.replace("{0}", moment(autoSavedContentDate).locale(editorInstance.config.language).format(editorInstance.lang.autosave.dateFormat));
            var jsonSavedContent = LoadData(autoSaveKey);

            if (current.length < 20) { // current is too short, load saved directly
                editorInstance.loadSnapshot(jsonSavedContent.data);
            } else {
