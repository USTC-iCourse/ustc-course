...
def view_profile(user_id):
    ...
    if user.is_profile_hidden and current_user != user and not (current_user.is_authenticated and current_user.is_admin):
        message = '此用户的个人主页未公开！'
        return render_template('feedback.html', status=False, message=message)
...
def follow_course(user_id):
    ...
    if not user or user.is_deleted:
        message = '用户不存在！'
        return render_template('feedback.html', status=False, message=message)
    if (user.is_profile_hidden or user.is_following_hidden) and current_user != user and not (current_user.is_authenticated and current_user.is_admin):
        message = '此用户关注的课程未公开！'
        return render_template('feedback.html', status=False, message=message)
